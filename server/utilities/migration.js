...
export async function findExternalUmbrelInstall() {
  try {
    ...
    const externalDrives = drives.filter(
      (drive) => drive.isUSB && !drive.isSystem
    );
...
export async function runPreMigrationChecks(
  ...
  const { version: previousVersion } = await fse.readJson(
    `${externalUmbrelInstall}/info.json`
  );
  const { version: currentVersion } = await fse.readJson(
    `${currentInstall}/info.json`
  );
  ...
  if (free < required) {
    throw new Error(
      `Not enough storage available. ${bytesToGB(free)} GB free, ${bytesToGB(
        required
      )} GB required.`
    );
  }
...
export async function migrateData(currentInstall, externalUmbrelInstall) {
  ...
  try {
    ...
    rsync.stdout.on("data", (chunk) => {
      ...
      if (progressUpdate) {
        ...
        if (progress > migrationStatus.progress)
    ...
    try {
      ...
      const files = await globby(
        `${temporaryData}/app-data/*/docker-compose.yml`
      );
      const pulls = [];
    ...
    await execa("./scripts/stop", ["--no-stop-server"], {
      cwd: currentInstall,
    });
    ...
    for (const path of statePaths) {
      ...
      if (await fse.pathExists(temporaryPath)) {
        await fse.move(temporaryPath, `${currentInstall}/${path}`, {
          overwrite: true,
        });
      }
    }
  ...
  await pRetry(
    () =>
      execa("./scripts/start", ["--no-start-server"], { cwd: currentInstall }),
    {
      retries: 5,
    }
  );
